...
/**
 * История и планы одним списком. Цвет здесь у метки, а не у строки: читателю
 * важно с одного взгляда отделить сделанное от обещанного, а не запомнить,
 * какая игра какого цвета.
 */
const ROADMAP: [RoadmapState, string, string][] = [
  [
    "done",
  ...
  ["done", "Мафия", "Роли на экранах, ночь и голосование без живого ведущего"],
  [
    "done",
    "Общая история",
  ...
  [
    "now",
    "Контакты",
    "Телеграм, почта и форма для багов: сейчас это заглушки, скоро заработают",
  ],
  [
    "next",
    "Новые игры",
...
const MARKS: Record<RoadmapState, [LucideIcon, string, string]> = {
  done: [Check, "Готово", "var(--alias-green)"],
  now: [Loader, "В работе", "var(--mf-gold)"],
  next: [Circle, "В планах", "var(--mf-text-faint)"],
};
    ...
    <PageShell active="О нас">
      ...
      <div className="pl-intro">
        <InkCard style={{ gap: 14 }}>
          ...
          <p className="pl-text">
            Компания собралась, вечер свободный, а игр с собой нет. И даже когда
            есть, приходится мириться: в Алиасе карточки в коробке рано или поздно
            заканчиваются, а в Мафии один человек весь вечер не играет, потому что
            он ведущий.
          </p>
          <p className="pl-text">
            Ради этого и сделана Игротека. Слова, роли, таймер и счёт берёт на себя
            сайт: в Алиасе больше восьми тысяч слов, и в одной партии они не
            повторяются, а в Мафии ночь сайт ведёт сам, вслух и по шагам. Нужны
            только телефоны и ссылка.
          </p>
        </InkCard>
      ...
      <section className="pl-section">
        ...
        <InkCard className="pl-roadmap">
          {ROADMAP.map(([state, title, text]) => {
            const [Icon, label, color] = MARKS[state];
            return (
              <div key={title} className="pl-roadmap-row">
                <span className="pl-roadmap-state" style={{ color }}>
                  <Icon size={15} /> {label}
                </span>
                <span
