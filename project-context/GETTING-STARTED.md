...
npm run setup:local       # клиент Prisma + базы + миграции + словарь
npm run dev               # web :3000 + ws :3001
```

Открыть **http://localhost:3000**.

`setup:local` делает четыре вещи: генерирует клиент Prisma, поднимает
контейнеры и **дожидается**, пока они станут healthy, применяет миграции,
заливает словарь. Это `db:generate`, `db:up`, `db:deploy` и `db:seed` подряд —
их можно вызывать и по отдельности.

Генерация идёт первой и до Docker: после свежего `npm install` клиента ещё нет
(`postinstall` в проекте не прописан), а без него падает `db:seed` — с
`@prisma/client did not initialize yet`. Заодно ошибка в схеме всплывает
раньше, чем поднимутся контейнеры.

### Какой словарь зальётся

`db:seed` ищет два файла, в таком порядке:

| Файл | Что это |
|---|---|
| `prisma/data/alias-catalog.json` | полный словарь, 8134 слова. В git его нет |
| `prisma/data/alias-catalog.sample.json` | демонстрационный набор, 109 слов |

Полный словарь — главный актив игры, поэтому в публичный репозиторий он не
входит. После свежего `git clone` рядом лежит только демонстрационный: три
подборки, три темы, три уровня сложности по двадцать слов. Играть на нём можно,
слова просто заканчиваются быстрее.

Сид печатает, какой из файлов взял, — так что «категории почти пустые» видно
сразу и не выглядит поломкой.

> **На демонстрационном наборе витрина и база расходятся — так и задумано.**
> Лендинг `/alias` статический, в базу не ходит: числа подборок написаны руками
> в `apps/web/src/constants/word-packs.ts` и описывают полный словарь. Поэтому
> на демо-наборе он покажет «Всего 8134 слов» вместо 109, уровни сложности — по
> тысяче с лишним слов вместо двадцати, и все восемь подборок вместо трёх
> залитых. Экран выбора категорий при этом честный: он читает базу, и счётчик
> слов там настоящий. Приводить витрину к демо-набору незачем — она описывает
> игру, а не тестовые данные.
...
prisma        схема, миграции, сид и демонстрационный словарь
```
...
**В категориях по двадцать слов**
Это не поломка: залит демонстрационный набор, потому что полного словаря рядом
нет. Так и выглядит проект после `git clone` — см. «Какой словарь зальётся».
Чтобы играть на полном, положите `alias-catalog.json` в `prisma/data/` и
повторите `npm run db:seed`.
