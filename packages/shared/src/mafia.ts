...
export interface MafiaSnapshot {
  ...
  pendingElim?: string;
  /**
   * Кто из живых нажал «пропустить обсуждение». Сошлись все — обсуждение
   * кончается досрочно.
   *
   * Раньше оборвать обсуждение мог только хост, и это подвешивало стол:
   * погибший хост уходит на экран наблюдателя, где кнопки нет вовсе, а
   * пропавший из сети не нажмёт её и подавно. Сбрасывается на входе в фазу.
   */
  discussionSkips?: string[];
  /**
...
export interface MafiaView {
  ...
  readyCount: number;
  aliveCount: number;
  /** Голоса за досрочный конец обсуждения. Есть только в фазе DISCUSSION. */
  discussionSkip?: {
    /** Сколько живых уже нажали. */
    count: number;
    /** Сколько живых всего — столько и нужно. */
    total: number;
    /** Нажал ли я. */
    mine: boolean;
  };
